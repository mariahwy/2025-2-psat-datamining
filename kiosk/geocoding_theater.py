...
def search_place(key, query):
    ...
    while current_page <= total_pages:
        ...
        try:
            ...
            # [수정됨] 로그 메시지에서 '이마트24' 카운트 삭제
            print(f"Page {current_page}/{total_pages} 처리 완료. {page_item_count_added}개 추가 (필터링: 서울주소 {page_item_count_filtered_seoul}개, 총 {len(all_items)}개)")
...
def main():
    ...
    theater_list = ["cgv", "메가박스", "롯데시네마"]
    
    all_results_list = []
    output_csv_filename = "theater_location.csv"
    
    for query in theater_list:
        try:
            print(f"\n---{query} 검색 시작---")
    ...
    if not all_results_list:
        ...
        try:
            ...
            results_df = pd.DataFrame(
                all_results_list, 
                columns=['theater', 'title', 'longitude', 'latitude']
            )
