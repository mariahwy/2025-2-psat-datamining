...
def search_place(key, query):
    ...
    while current_page <= total_pages:
        ...
        try:
            ...
            page_item_count_filtered = 0 # 2. '서울'이 아니라서 필터링된 개수
            # -------------------------------------

            for item in result['items']:
                title = item.get('title', 'N/A')
                
                # --- [신규] 도로명 주소 추출 및 필터링 로직 ---
                # 1. item['address']['road'] 값을 안전하게 가져옵니다.
                # .get('address', {})는 'address' 키가 없으면 빈 딕셔너리 {} 를 반환
                # .get('road', '')는 'road' 키가 없으면 빈 문자열 "" 을 반환
                road_address = item.get('address', {}).get('road', '') 
                
                # 2. 도로명 주소에 "서울"이 포함되어 있는지 확인
                if "서울" in road_address:
                    ...
                    page_item_count_filtered += 1
                # --- [신규] 필터링 로직 끝 ---

            # [수정됨] 로그 메시지를 더 상세하게 변경
            print(f"Page {current_page}/{total_pages} 처리 완료. {page_item_count_added}개 추가 (필터링: {page_item_count_filtered}개, 총 {len(all_items)}개)")
...
def main():
    ...
    filename = "franchise.csv"
    try:
        df = pd.read_csv(filename)
    except Exception as e:
        print(f"kiosk file을 읽을 수 없습니다. : {e}")
    
    all_results_list = []
    output_csv_filename = "franchise_location.csv"
    
    pattern = re.compile(r'[^가-힣 ]')
    
    try:
        for original_query in df["영업표지"]:
            # 정규화
            query = pattern.sub('', str(original_query))
            query = query.strip()
            query = re.sub(r'\s+', ' ', query) 
            
            if not query:
                print(f"\n--- [SKIP] 원본 '{original_query}' -> 정제 후 빈 문자열이 되어 건너뜁니다.")
                continue
    ...
    if not all_results_list:
        ...
        try:
            ...
            results_df = pd.DataFrame(
                all_results_list, 
                columns=['franchise', 'title', 'longitude', 'latitude']
            )
