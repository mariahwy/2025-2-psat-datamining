...
def search_place(key, query):
    ...
    while current_page <= total_pages:
        ...
        try:
            ...
            # --- [수정됨] 카운터 3종 ---
            page_item_count_added = 0
            page_item_count_filtered_seoul = 0
            page_item_count_filtered_emart24 = 0 # [수정] 'ATM' 카운터 삭제
            # -------------------------------------
            ...
            for item in result['items']:
                ...
                # --- [신규] 1순위 필터: "이마트" 검색 시 "이마트24" 제외 ---
                if query == "이마트" and "이마트24" in title:
                    page_item_count_filtered_emart24 += 1
            ...
            # [수정됨] 로그 메시지에서 'ATM' 카운트 삭제
            print(f"Page {current_page}/{total_pages} 처리 완료. {page_item_count_added}개 추가 (필터링: 서울주소 {page_item_count_filtered_seoul}개, 이마트24 {page_item_count_filtered_emart24}개, 총 {len(all_items)}개)")
...
def main():
    ...
    market_list = ["이마트", "홈플러스", "롯데마트"]
    
    all_results_list = []
    output_csv_filename = "supermarket_location.csv"
    ...
    if not all_results_list:
        ...
        try:
            ...
            results_df = pd.DataFrame(
                all_results_list, 
                columns=['market', 'title', 'longitude', 'latitude']
            )
